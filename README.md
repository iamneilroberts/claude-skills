...
| [`comment-refactor`](skills/comment-refactor/SKILL.md) | Strip comment cruft from AI-generated code — fix-history narration, obsolete explanations, code-restating noise — keeping load-bearing WHY/TODO/warnings. Whole-repo or a path/glob; skips dead/vendored/generated files; a content-hash marker makes re-runs skip unchanged files. The code counterpart to `unslop-ui`. |
